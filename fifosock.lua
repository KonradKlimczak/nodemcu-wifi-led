...
local function wrap(sock)
    ...
    local function sendnext() if not fbig:dequeue(ssend) then promote() end end
    ...
    return function(s)
        ...
        local corked = false
        local function corker(t)
            corked = true;
            return t
        end

        -- small fifo would overfill?  promote it
        if lsmall + #s > BIGTHRESH or #fsmall >= FSMALLLIM then
            promote(corker)
        end
        ...
        elseif fbig._go and lsmall == 0 then
            fbig:queue(s, corker)
        ...
        --   before transmitting.
        if corked then
            if #fbig <= COALIMIT then
                tmr.create():alarm(1, tmr.ALARM_SINGLE, sendnext)
            else
                sendnext()
            end
        end
