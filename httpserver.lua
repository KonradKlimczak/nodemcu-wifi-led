...
do
    ...
    local make_req = function(conn, method, url)
        return {conn = conn, method = method, url = url}
    end
    ...
    local make_res = function(csend, cfini)
        ...
        local finish = function(self, data, status)
            ...
            if data then self:send(data, status) end
    ...
    local http_handler = function(handler)
        return function(conn)
            ...
            local onheader = function(connection, k, v) -- luacheck: ignore
                ...
                if req and req.onheader then req:onheader(k, v) end
            end
            -- body data handler
            local body_len = 0
            local ondata = function(connection, chunk) -- luacheck: ignore
                ...
                if body_len >= cnt_len then req:ondata() end
            end
